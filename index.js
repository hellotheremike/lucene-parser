...
//             var sValue = sParsedTerm[1];
//             var sKey = sParsedTerm[0];
//             console.log(sParsedTerm)
...
LuceneParser.SanitizePatterns = {
    'LUCENE': {
        ...
        'parse': function(sTerm) {
            var sRegexp = LuceneParser.SanitizePatterns['LUCENE']['pattern'];
            var sParsedTerm = sTerm.match( sRegexp );
...
LuceneParser.prototype._luceneTemplateSearchTerm = function() {
    ...
    */
    ...
    _.forEach( oPatterns, function(rxPattern) {
        this.sFormattedSearchTerm = this.sFormattedSearchTerm.replace( rxPattern, function(sTerm) {
            that.oLuceneTemplateCache['_' + nTemplateCount] = sTerm;
...
LuceneParser.prototype._sanitizer = function() {
    ...
    _.forEach( rTerms, function(sTerm) {
        ...
        if (sTerm === '*') {
            ...
            this.sSanitized += this.oLuceneTemplateCache['_' + rTemplateMatch[1]] + ' ';
        } else if (oMatch) {
